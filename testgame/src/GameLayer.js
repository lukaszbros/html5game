...
var GameLayer = cc.Layer.extend({
    ...
    init:function () {
        ...
        socket.on('oponents', function(data){
        	console.log('oponents');
        	console.log(data);
        	//add allready connected ponents
        	data.forEach(function(element, index, array) {
        		console.log(element);
        		console.log(socket);
        		if ((g_sharedGameLayer._shipOponent[element] == null)  &&  (element.toString() != socket.socket.sessionid))
        		{
        			console.log('add oponent');
        			console.log(element);
        			// ship oponent
		   			g_sharedGameLayer._shipOponent[element] = new Ship();
		   			g_sharedGameLayer._texTransparentBatch.addChild(g_sharedGameLayer._shipOponent[element], g_sharedGameLayer._shipOponent[element].zOrder, MW.UNIT_TAG.PLAYER);
		   		}	
        	});
		});
        socket.on('oponent_connected', function(data){
        	console.log('oponent_connected');
        	console.log(data);
			// ship oponent
		   	g_sharedGameLayer._shipOponent[data.oponentId] = new Ship();
		   	g_sharedGameLayer._texTransparentBatch.addChild(g_sharedGameLayer._shipOponent[data.oponentId], g_sharedGameLayer._shipOponent[data.oponentId].zOrder, MW.UNIT_TAG.PLAYER);	
		});
		socket.on('oponent_disconnected', function(data){
        	console.log('oponent_disconnected');
        	console.log(data);
        	
		   	g_sharedGameLayer._texTransparentBatch.removeChild(g_sharedGameLayer._shipOponent[data.oponentId]);	
		});
		socket.on('shipoponent_move', function(data){
			// ship oponent move
			console.log(data);
			console.log(g_sharedGameLayer._shipOponent);
		   	g_sharedGameLayer._shipOponent[data.oponentId].setPosition( data.position );
		});
        return bRet;
    },
    ...
    processEvent:function( event ) {
        if( this._state == STATE_PLAYING ) {
    		...
    		socket.emit('ship_move', { position: curPos });
